...
## Last Updated
2026-09-11 20:50 IST — Win32 Occlusion & Mute Policy Engine Overhaul:
1. Fixed premature `EnumWindows` aborting: Filtered and counted up to 120 genuine visible candidate application windows across all connected monitors instead of stopping on invisible/cloaked background windows.
2. Self-Process PID Exclusion: Filtered all AetherFlow windows via `GetWindowThreadProcessId` against `std::process::id()`.
3. Modern Fullscreen & Maximized Rect Detection: Removed obsolete `!has_caption` constraint (which failed on Antigravity/Chromium/VS Code/borderless apps); any window filling the physical display is accurately identified as fullscreen. Maximized windows are detected via `IsZoomed` or work area boundaries (`rcWork` with 15px invisible shadow frame tolerance).
4. Audio Policy Refinement: In `Mute When Covered` mode, audio properly evaluates true screen occlusion (`is_fullscreen || is_maximized`) rather than focus. Added periodic diagnostic logging (`[SYSTEM MONITOR DIAG]`) every 6s in `desktop_debug.log`. Set default `pauseOnMaximized: true`.
5. Verified `npm run build` (491ms) and `cargo check` (0 warnings/errors), compiled release binary, and updated root `.\AetherFlow.exe` (7.09 MB).
