...
## Last Updated
2026-09-11 21:45 IST — Resolved Self-Occlusion Bug in MPV Video Engine & Global/Isolated State Desync:
1. **Self-Occlusion Root Cause Fixed**: MPV child processes (`AetherFlow-VideoEngine.exe`) run under their own PIDs and HWNDs. Because previous filtering only checked `self_pid`, MPV's own wallpaper window was falsely identified by `enum_occlusion_proc` as a fullscreen/maximized application covering the monitor. This permanently locked `any_monitor_covered = true`, paralyzing Global mode and causing stale state to poison Isolated mode.
2. **Comprehensive Exclusion Filters**: Excluded all MPV PIDs (`mpv_pids`), MPV HWNDs (`mpv_hwnds`), webview wallpaper HWNDs (`wallpaper_hwnds`), Progman/WorkerW child/ancestor hierarchies (`GetAncestor(GA_ROOT / GA_ROOTOWNER)`), transparent overlay styles (`WS_EX_TRANSPARENT`), and window classes (`mpv`, `WorkerW`, `Progman`, `SHELLDLL_DefView`, `SysListView32`).
3. **Instant Setting Sync & State Flush**: Added atomic `MONITOR_SYNC_REQUESTED` flag triggering immediate cleanup of `paused_monitors` and re-evaluation whenever users toggle settings, switch multi-monitor modes, or change wallpapers.
4. **Secondary Monitor Audio Unmuting**: Fixed `set_mpv_mute` to unmute the active video wallpaper even when assigned solely to a secondary display (e.g. `DISPLAY6`), preventing invalid muting of single-engine secondary setups.
5. **Mode-Aware Audio Muting**: In `per-display` mode with `mute-covered`, audio only mutes if the specific monitor playing audio is covered (or all monitors paused), while in `all-displays` mode, any covered monitor triggers global mute.
6. Verified with `cargo check` (0 warnings/errors), `npm run build` (704ms), compiled release binary, and updated root `.\AetherFlow.exe` (7.09 MB).
